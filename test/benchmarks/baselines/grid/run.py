...
def _per_algo_batch_main_source(floating_base: bool | None = None,
    ...
    return (
        ...
        "// loops over N in {16, 32, 64, 128, 256, 1024} to match timeGRiD_batch.cu.\n"
        "#include \"timeGRiD_common.h\"\n"
        "\n"
        + "".join(decls)
        + "\n"
        ...
        "int main(int argc, const char **argv){\n"
        "    bool floating_base = parse_floating_base_arg(argc, argv);\n"
        "    // MAX_TIMESTEPS sizes the device buffers (gridData) — must be >= the\n"
        "    // largest batch N below (now 1024) or N=1024 launches go out of bounds.\n"
