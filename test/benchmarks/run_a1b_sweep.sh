...
# g1 / h2_plus) are now ~6-7 GB of cicc EACH. The default auto build-jobs (~8 parallel)
...
BUILD_JOBS="${1:-4}"
# A1B_ROBOTS env overrides the robot set. NOTE: h2_plus's SO monolithic compile uses
# ~36 GB RAM (single cicc) — exclude it unless build-jobs=1 + the box has the RAM,
# or until the subset-timing harness (B7) lets us build it dynamics-only. h2_plus
# is GRiD-internal (no mjx/frax/pinocchio/cuRobo model) so its competitor columns
# are skipped by run_multi_version automatically.
