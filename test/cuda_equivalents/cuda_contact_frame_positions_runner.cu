...
//   grid::contact_frame_positions_device            -> 3*NUM_CONTACT_FRAMES world positions of the
//                                                     baked contact ORIGINS (the f_ext_body wrench points)
//   grid::contact_frame_positions_gradient_device   -> 3*NUM_VEL per frame tangent Jacobian
//                                                     [3*NUM_VEL*f + 3*vi + row], tangent [v_lin; omega; joints]
//   grid_plant::contact_frame_positions[_gradient]  -> the caller-scratch wrappers (must equal the device fns)
...
constexpr int NF = grid::NUM_CONTACT_FRAMES;
...
__global__ void pos_kernel(T *d_pos, const T *d_q, const grid::robotModel<T> *m) {
    ...
    grid::contact_frame_positions_device<T>(s_pos, d_q, m);
    __syncthreads();
...
__global__ void pos_kernel_spill(T *d_pos, const T *d_q, const grid::robotModel<T> *m, T *d_ws) {
    __shared__ T s_pos[3*NF];
    grid::contact_frame_positions_device<T, grid::TIER_MINIMAL>(s_pos, d_q, m, d_ws);
    __syncthreads();
    if (threadIdx.x == 0) for (int i = 0; i < 3*NF; ++i) d_pos[i] = s_pos[i];
}
__global__ void grad_kernel(T *d_grad, const T *d_q, const grid::robotModel<T> *m) {
    __shared__ T s_grad[3*NV*NF];
    grid::contact_frame_positions_gradient_device<T>(s_grad, d_q, m);
    __syncthreads();
...
__global__ void plant_kernel(T *d_pos, T *d_grad, const T *d_q, const grid::robotModel<T> *m) {
    __shared__ T s_pos[3*NF];
    __shared__ T s_grad[3*NV*NF];
    __shared__ __align__(16) T s_scratch[grid::CONTACT_FRAME_POSITIONS_GRADIENT_DYNAMIC_SHARED_MEM_COUNT];
    grid_plant::contact_frame_positions_gradient<T>(s_pos, s_grad, d_q, s_scratch, m);
    __syncthreads();
    if (threadIdx.x == 0) {
...
__global__ void plant_pos_kernel(T *d_pos, const T *d_q, const grid::robotModel<T> *m) {
    __shared__ T s_pos[3*NF];
    __shared__ __align__(16) T s_scratch[grid::CONTACT_FRAME_POSITIONS_DYNAMIC_SHARED_MEM_COUNT];
    grid_plant::contact_frame_positions<T>(s_pos, d_q, s_scratch, m);
    __syncthreads();
    if (threadIdx.x == 0) for (int i = 0; i < 3*NF; ++i) d_pos[i] = s_pos[i];
}
...
int main(){
    ...
    size_t smem_p = grid::CONTACT_FRAME_POSITIONS_DYNAMIC_SHARED_MEM_BYTES<T>();
    size_t smem_g = grid::CONTACT_FRAME_POSITIONS_GRADIENT_DYNAMIC_SHARED_MEM_BYTES<T>();
    ...
    size_t smem_spill = grid::CONTACT_FRAME_POSITIONS_DYNAMIC_SHARED_MEM_BYTES<T, grid::TIER_MINIMAL>();
    size_t ws_bytes   = grid::CONTACT_FRAME_POSITIONS_DEVICE_INLINE_WORKSPACE_BYTES<T, grid::TIER_MINIMAL>();
